...
export function expandBank(
  ...
  const angles =
    ...
    ([
      ...
      (base) =>
        `Describe a real-world example related to: ${base.replace(/\?$/, '').toLowerCase()}?`,
      (base) =>
        `What would you do differently today regarding: ${base.replace(/\?$/, '').toLowerCase()}?`,
      (base, n) =>
        `How did this scale across multiple clients (part ${n})? ${base.replace(/\?$/, '')}?`,
  ...
  for (let i = 0; i < count; i++) {
    ...
    const q = cycle === 0 ? item.q : angles[(cycle - 1) % angles.length](item.q, i + 1);
